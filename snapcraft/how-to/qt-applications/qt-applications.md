...
This is because Qt 5 applications expect to learn about the cursor theme and size from two environment variables: `XCURSOR_THEME` and `XCURSOR_SIZE`. Some desktop environments (e.g. GNOME) do not set these values, and as a result Qt 5 doesn't know which cursor theme or size to use, and instead falls back to default settings.

As a workaround, we can insert a few lines into the snap's built-in initialisation script which set these values from D-Bus whenever our snap is run. We're doing this using another part named `get-cursor-from-dbus`:
...
This works similar to the `sound-support` part mentioned above, in that it uses `plugin: nil` and `override-prime` to run a custom script. The biggest difference is that we set the `after` attribute to state that our `sound-support` part must be run `after` the `kde-neon/sdk` (which is added to *snapcraft.yaml* automatically at build time by the `kde-neon` extension). The reason for this is that `kde-neon/sdk` adds the initialisation script that our `get-cursor-from-dbus` part modifies.

> TO DO: This works on a Ubuntu 24.04 host running GNOME via Wayland. KDE 5.27 via Wayland (at least as installed on Debian 12) sets these variables for us. XFCE uses X11. What about other DEs/compositors? Could test LXDE-Pi to confirm arm64 support...
...
Here's the updated *snapcraft.yaml* for our snap. If you cloned the *kcalc-example* repository from GitHub earlier on, then you should find it in the *revised* folder of that repository.
